...
class ReferenceExtractor:
    """Extractor-protocol wrapper around ``extract_and_validate``.

    Deterministic, offline, no API key. Used as the fallback whenever
    Groq is unavailable, and as the only extractor the test suite runs
    ...
    def extract(self, segments: list[TranscriptSegment], meeting_id: str) -> list[ValidatedItem]:
        return extract_and_validate(segments, meeting_id)
